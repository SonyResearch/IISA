...
def fit_curve(x, y, curve_type='logistic_4params'):
    ...
    """
    assert curve_type in ['logistic_4params', 'logistic_5params'], (
...
@METRIC_REGISTRY.register()
def calculate_log_rmse(x, y, fit_scale=None, eps=1e-8):
    if fit_scale is not None:
        x = fit_curve(x, y, fit_scale)
    return np.sqrt(np.mean((np.log(x + eps) - np.log(y + eps)) ** 2))
...
@METRIC_REGISTRY.register()
def calculate_log_mae(x, y, eps=1e-8):
    return np.mean(np.abs(np.log(x + eps) - np.log(y + eps)))
