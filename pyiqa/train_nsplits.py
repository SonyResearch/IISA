...
def train_nsplits(root_path):
    ...
    save_path = f"{opt['save_final_results_dir']}/{opt['name']}/{n_splits}splits_results.txt"
    ...
    with open(save_path, 'w') as sf:
        ...
        for ds in datasets:
            ...
            results_median = np.median(np.array(all_results), axis=0)
            results_avg = np.array(all_results).mean(axis=0)
            results_std = np.array(all_results).std(axis=0)
            sf.write(f'Overall results in {ds}: {results_median}\t{results_avg}\t{results_std}\n')
